...
struct SignUpView: View {
    ...
    var body: some View {
        NavigationStackView(transitionType: .default) {
            NavigationLink(destination: ConfirmEmailView(), isActive: $action) {
                EmptyView()
            }
            VStack {
                // navigation links
                ...
                VStack(alignment: .center) {
                    ...
                    Button(action: {
                        //login
                        let sign = viewModel.signup(email: email, password: password, username: username, phoneNum: phonenumber)
                        if (sign[0] as! Bool)
                        {
                            //viewModel.confirmEmail(email: email, password: password, username: username, phoneNum: phonenumber)
                            self.action.toggle()
                        }
                        else {
